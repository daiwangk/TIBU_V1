...
setSavedReels,
  ...
  const { data: crochetReels = [], loading: load_crochetReels } = useAsync(() => listReels('crochet'));
...
const [filters, setFilters] = useState({
  ...
  if (load_crochetProducts || load_crochetBusinesses || load_crochetReels) return <div style={{padding: "40px", textAlign: "center"}}>Loading...</div>;
const filteredCrochetProducts = crochetProducts.filter((item) => {
  const matchesSearch =
...
>
  ...
  minWidth: "169px",
  height: "82px",
  background: item.name === "Crochet" ? "#5A1848" : "#fff",
  color: item.name === "Crochet" ? "#fff" : "#35142E",
  borderRadius: "22px",
  display: "flex",
...
filters.price === "All" && (
  <>
<Banner
  title="Beautiful Crochet Creations 🧶"
  subtitle="Flowers • Plushies • Bags • Keychains"
  emoji="🧶"
  color1="#8E4D6D"
  color2="#F7BFA5"
/>
...
>
  Flowers • Plushies • Bags • Keychains • Home Décor
</p>
{/* QUICK SEARCH */}
<div
  style={{
...
>
  ...
  <button
  onClick={() => {
    setViewAllTitle("🧶 Crochet Near You");
    setViewAllProducts(crochetProducts.slice(0, 3));
    setPreviousPage("crochet");
...
>
  ...
  onClick={() => {
    ...
    setViewAllProducts(crochetProducts.slice(3, 6));
    setPreviousPage("crochet");
    setPage("productsviewall");
  }}
    style={{
...
>
  {crochetProducts.slice(3, 6).map((shop, index) => (
   <ProductCard
  key={index}
  product={shop}
  name={shop.name}
...
>
  ...
  setViewAllBusinesses(crochetBusinesses);
  setPreviousPage("crochet");
...
>
  {crochetBusinesses.map((business, index) => (
 <BusinessCard
  business={business}
...
>
  ...
  <button
  onClick={() => {
  setViewAllReelsTitle("🎥 Crochet Reels");
  setViewAllReels(crochetReels);
  setPreviousPage("crochet");
...
{(
  ...
  <>
    ...
    >
      {filteredCrochetProducts.length} product
      {filteredCrochetProducts.length !== 1 ? "s" : ""} found
    </p>
    ...
    >
      {filteredCrochetProducts.map((shop, index) => (
        <ProductCard
          key={index}
          name={shop.name}
          seller={shop.seller}
      ...
      {filteredCrochetProducts.length === 0 && (
        <p
          style={{
            textAlign: "center",
            color: "#777",
