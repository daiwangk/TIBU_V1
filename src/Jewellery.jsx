...
setSavedReels,
  ...
  const { data: jewelleryReels = [], loading: load_jewelleryReels } = useAsync(() => listReels('jewellery'));
...
const [filters, setFilters] = useState({
  ...
  if (load_jewelleryProducts || load_jewelleryBusinesses || load_jewelleryReels) return <div style={{padding: "40px", textAlign: "center"}}>Loading...</div>;

const filteredJewelleryProducts = jewelleryProducts.filter((item) => {
  const matchesSearch =
...
<SearchBar
  placeholder="Search rings, earrings..."
...
>
  ...
  minWidth: "169px",
  height: "82px",
  background: item.name === "Jewellery" ? "#5A1848" : "#fff",
  color: item.name === "Jewellery" ? "#fff" : "#35142E",
  borderRadius: "22px",
  display: "flex",
...
<Banner
  ...
  subtitle="Rings • Earrings • Bracelets • Necklaces"
  emoji="💍"
  color1="#8E4D6D"
  color2="#F7BFA5"
/>
...
>
  ...
  <button
   onClick={() => {
    setViewAllTitle("💍 Jewellery Near You");
    setViewAllProducts(jewelleryProducts.slice(0, 3));
    setPreviousPage("jewellery");
...
>
  ...
  onViewProduct={() => {
    ...
    const business = jewelleryBusinesses.find(
      (b) => b.id === shop.businessId
    );
...
>
  ...
  <button
   onClick={() => {
    ...
    setViewAllProducts(jewelleryProducts.slice(3, 6));
    setPreviousPage("jewellery");
    setPage("productsviewall");
  }}
    style={{
...
>
  {jewelleryProducts.slice(3, 6).map((shop, index) => (
    <ProductCard
  key={index}
  product={shop}
  name={shop.name}
...
>
  ...
  setViewAllBusinesses(jewelleryBusinesses);
  setPreviousPage("jewellery");
...
>
  {jewelleryBusinesses.map((business, index) => (
<BusinessCard
  business={business}
...
>
  ...
  <button
  onClick={() => {
  setViewAllReelsTitle("🎥 Jewellery Reels");
  setViewAllReels(jewelleryReels);
  setPreviousPage("jewellery");
...
>
  ...
  <ReelCard
    ...
    onClick={() => {
      ...
      setCurrentReels(jewelleryReels);
      setPage("reel");
    }}
  />
))}
...
{(
  ...
  <>
    ...
    >
      {filteredJewelleryProducts.length} product
      {filteredJewelleryProducts.length !== 1 ? "s" : ""} found
    </p>
    ...
    >
      {filteredJewelleryProducts.map((shop, index) => (
        <ProductCard
          key={index}
          name={shop.name}
          seller={shop.seller}
